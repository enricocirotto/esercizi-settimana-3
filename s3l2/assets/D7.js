...
concatenaStringhe("giorno", "notte");
...
const arrayPari = filtraPari(arrayNumerico);
...
const arrayDiNumerii = [1, 2, 3, 4, 5];
const risultatoSommaa = sommaArrayConForEach(arrayDiNumerii);

console.log(risultatoSommaa);
...
const arrayIncrementato = incrementaArray(arrayNumeri, numeroIncremento);
...
const arrayDiStringhe = ["marte", "luna", "andromeda"];
...
/* function trovaFilmPiuVecchio(movies) {
  ...
  movies.forEach(film => {
      ...
      if (!isNaN(annoFilm) && (filmPiuVecchio === null || annoFilm < parseInt(filmPiuVecchio.Year))) {
          filmPiuVecchio = film;
      }
  });
...
function sommaAnniDiProduzione(movies) {
  return movies.reduce((somma, film) => {
      const annoFilm = parseInt(film.Year);
      return !isNaN(annoFilm) ? somma + annoFilm : somma;
  }, 0);
}


const risultatoSommaAnni = sommaAnniDiProduzione(movies);
